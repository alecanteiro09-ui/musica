...
/** Graba, revisa y envía un clip corto de audio. onDone recibe el Blob grabado. */
function useRecorder() {
  ...
  async function start() {
    ...
    try {
      ...
      setError("No pudimos acceder al micrófono. Revisa el permiso del navegador.");
    }
  }
...
function RecordStep({
  ...
  // Para solo al llegar al límite — graba solo lo necesario, sin rodeos,
  // para validar y clonar más rápido (upload menor, menos audio pa Kie.ai).
  useEffect(() => {
      ...
      {!rec.blob ? (
        <div className="mt-8 flex flex-col items-center gap-3">
          <button
            ...
            aria-label={rec.recording ? "Detener grabación" : "Empezar a grabar"}
          >
            {rec.recording ? <Square size={22} /> : <Mic size={24} />}
          </button>
          <p className="text-xs text-ink-muted">
            {rec.recording ? `Grabando... ${rec.seconds}s / ${maxSeconds}s` : `Toca para grabar (${minSeconds}-${maxSeconds}s)`}
          </p>
        </div>
      ) : (
...
export function VoiceRecorder({
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      ...
      if (result.status === "failed") {
        setError(result.error || "No pudimos validar esa muestra.");
        setStep("failed");
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      ...
      if (result.status === "ready") {
        ...
        setError(result.error || "No pudimos clonar esa voz.");
        setStep("failed");
      }
    }, 1200);
    return () => clearInterval(poll);
  ...
  return (
    <div className="mx-auto max-w-xl px-6 py-16">
      <p className="text-center text-sm uppercase tracking-wide text-accent">tu voz en la canción</p>
      <h1 className="mt-2 text-center font-display text-2xl italic text-ink">Vamos a clonar tu voz</h1>

      <div className="mt-8 rounded-xl border border-base-border bg-base-soft p-6">
        {step === "intro" && (
          <RecordStep
            title="Graba un fragmento cantando"
            subtitle="Solo unos 8-12 segundos, en el lugar más silencioso posible. Corto y directo valida más rápido."
            minSeconds={8}
            maxSeconds={12}
            busy={busy}
        ...
        {(step === "awaiting_phrase" || step === "uploading_sample") && (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <p className="text-sm text-ink-muted">Analizando tu voz... normalmente toma menos de 1 minuto.</p>
          </div>
        )}

        {step === "recording_reading" && phrase && (
          <RecordStep
            title="Ahora lee (o canta) esta frase"
            subtitle={`"${phrase}"`}
            minSeconds={2}
            maxSeconds={7}
            busy={busy}
        ...
        {(step === "processing" || step === "uploading_reading") && (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            <Loader2 size={22} className="animate-spin text-accent" />
            <p className="text-sm text-ink-muted">Confirmando que eres tú y clonando tu voz... ya casi.</p>
          </div>
        )}

        {step === "ready" && (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            <p className="text-sm font-medium text-success">¡Tu voz está lista! Preparando el siguiente paso...</p>
      ...
      {step !== "ready" && (
        ...
        >
          <SkipForward size={14} /> Prefiero seguir con la voz estándar de la IA
        </button>
      )}
    </div>
  );
