...
export function ChorusPicker({
  ...
  return (
    <div className="mx-auto max-w-xl px-6 py-16">
      <p className="text-center text-sm uppercase tracking-wide text-accent">tu letra, a tu manera</p>
      <h1 className="mt-2 text-center font-display text-2xl italic text-ink">¿Cuál coro te gusta más?</h1>
      <p className="mt-2 text-center text-sm text-ink-muted">
        Es la parte que más se canta. Elige la que te llegue — se puede ajustar todo después.
      </p>
      ...
      <div className={cn("mt-8 flex flex-col gap-4 transition-opacity", isRegenerating && "opacity-40")}>
        {options.map((opt, i) => (
          ...
          >
            <span className="mb-2 block text-xs uppercase tracking-wide text-ink-muted">Opción {i + 1}</span>
            {opt.content.split("\n").map((line, j) => (
      ...
      >
        ...
        {isRegenerating ? "Reescribiendo con IA..." : "No me gustó ninguno — reescribir con IA"}
      </button>
      ...
      >
        {isPending ? `Escribiendo la letra de ${nickname || "tu canción"}...` : "Usar este coro"}
      </button>
    </div>
  );
}
