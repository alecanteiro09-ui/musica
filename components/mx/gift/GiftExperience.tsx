...
import { AudioPlayer } from "@/components/gift/AudioPlayer";
import { KaraokeLyrics } from "@/components/gift/KaraokeLyrics";
import { QrCode } from "@/components/gift/QrCode";
...
export function GiftExperience({
  ...
  if (!track) {
    return <p className="text-center text-ink-muted">Este regalo todavía no tiene audio disponible.</p>;
  }
    ...
    <div className="relative min-h-screen" style={{ background: "#1A1420" }}>
      ...
      {!revealed && (
        <div className="relative z-10 flex min-h-screen w-full flex-col items-center justify-center px-6 text-center">
          ...
          <h1 className="mt-4 max-w-md font-display text-4xl italic text-[#FBF7FA] sm:text-5xl">
            Una canción para {gift.nickname}
          </h1>
          <p className="mt-3 text-sm text-[#C9BBCE]">Toca para escuchar</p>
          <button
            onClick={openGift}
            aria-label="Reproducir la canción"
      ...
      {revealed && (
        <div className="relative z-10 mx-auto max-w-xl px-6 py-16" style={{ animation: "rise-in 0.7s ease both" }}>
          <p className="text-center text-sm uppercase tracking-wide text-accent">una canción para</p>
          ...
          <div className="mt-12 flex flex-col items-center gap-6 border-t border-white/10 pt-8">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-[#FBF7FA]/50">Apunta la cámara para abrir este regalo de nuevo</p>
            </div>
          </div>

          <p className="mt-10 text-center text-xs text-[#FBF7FA]/40">Hecho con Verso Único</p>
        </div>
      )}
    </div>
  );
