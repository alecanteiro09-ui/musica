  ...
  async function handleSaveToPhone() {
    ...
    try {
      ...
      if (canShareFiles) {
        ...
        const file = new File([blob], `cancion-para-${nickname || "ti"}.mp3`, { type: "audio/mpeg" });
        if (navigator.canShare({ files: [file] })) {
          await navigator.share({ files: [file], title: `Una canción para ${nickname}` });
          setSharing(false);
          return;
        }
      }
    } catch {
      // el usuario canceló el share nativo o el navegador lo rechazó — cae al descarga directa
    }
    const a = document.createElement("a");
    a.href = audioUrl;
    a.download = `cancion-para-${nickname || "ti"}.mp3`;
  ...
  function handleSendEmail(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        ...
        setEmailState("error");
        setErrorMsg(result.error || "No pudimos enviarlo ahora.");
      }
    });
  }
        ...
        <form onSubmit={handleSendEmail} className="mt-3 flex flex-col gap-2" style={{ animation: "rise-in 0.3s ease both" }}>
          {emailState === "sent" ? (
            <p className="flex items-center gap-2 rounded-xl bg-success/15 px-4 py-3 text-sm text-success">
              <Check size={16} /> ¡Enlace enviado! Revisa la bandeja de entrada de {email}.
            </p>
          ) : (
            <>
